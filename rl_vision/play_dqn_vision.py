...
# Si el entorno busca la estructura de NumPy 2.x, la redirigimos a la 1.x que usamos en el entrenamiento
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run a trained vision DQN agent for Space Invaders.")
    parser.add_argument(
        "--model-path",
        type=str,
...
def main():
    ...
    model_probe = DQN.load(str(model_file))
    ...
    model = DQN.load(str(model_file), env=env)
    ...
    for episode in range(1, args.episodes + 1):
        ...
        while not done:
            # En DQN, args.stochastic controlará si se aplica política determinista 
            # o si se añade la exploración épsilon residual en la inferencia.
